...
class FunctionalAnalysis:
    ...
    def perform_analysis(self, clustering_data: Dict[str, Any], output_file: str, algorithm: str = None):
        ...
        try:
            ...
            if all_results:
                final_results = pd.concat(all_results, ignore_index=True).drop(columns=["Old P-value", "Old Adjusted P-value"], errors="ignore")
    ...
    def filter_results(self, input_file: str, output_file: str, 
        ...
        try:
            ...
            # Validar que se proporciona al menos un criterio de filtrado
            if p_value_threshold is None and combined_score_min is None and overlap_percentage_min is None:
                raise ValueError("Debes proporcionar al menos un criterio de filtrado (p_value_threshold, combined_score_min o overlap_percentage_min).")

            # Crear una nueva columna de Overlap_Percentage si se proporciona el umbral
