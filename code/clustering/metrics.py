...
class Metrics:
    @staticmethod
    def modularity(graph: Graph, clusters: List[List[int]], logger: logging.Logger) -> Optional[float]:
        """
    ...
    @staticmethod
    def functional_enrichment_score(graph: Graph, clusters: List[List[int]], logger: logging.Logger) -> float:
        """
        ...
        try:
            ...
            for i, cluster in enumerate(clusters):
                ...
                enrichment_results = Metrics._perform_enrichment(genes=genes, logger=logger)
            ...
            if scores:
                ...
                logger.info(f"Puntaje promedio de enriquecimiento funcional: {average_score}")
    ...
    def _perform_enrichment(genes: List[str], logger: logging.Logger) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Realizando análisis de enriquecimiento para los genes: {genes}")

            # Obtener IDs de STRINGdb
            string_ids = get_string_ids(genes)
            if string_ids.empty:
                logger.warning("No se encontraron IDs válidos para los genes proporcionados.")
                return None
            ...
            if filtered_terms.empty:
                logger.warning("No se encontraron términos enriquecidos para los genes proporcionados.")
                return None

            logger.info(f"Resultados de enriquecimiento obtenidos: {len(filtered_terms)} términos enriquecidos.")
