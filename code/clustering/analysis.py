...
def get_extreme_configurations(
    ...
    if len(params_columns) != 1:
        raise ValueError(
            "Expected exactly one 'params_*' column in the dataset, found: {}".format(
                params_columns
            )
        )
...
def plot_saved_clustering_results(
    saved_clustering_paths: Dict[str, Dict[float, Tuple[str, str]]],
    base_path_plots: os.PathLike,
) -> None:
    """
    ...
    fig, axs = plt.subplots(
        1, num_params, figsize=(5 * num_params, 5), constrained_layout=True
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Visual analysis of clustering results"
    )
    ...
    if VERBOSE:
        print(
            f"Starting clustering analysis. Results are being plotted to {PATH_PLOTS}"
        )
    ...
    if VERBOSE:
        print(
            f"INFO: Folder {output_dir} is a temp folder, it will be removed shortly."
        )
    ...
    for algorithm_name, (
        ...
        (values_0_a, values_1_a), (param_value_a, param_name_a) = list(
            extreme_configurations.items()
        )[0]
        (values_0_b, values_1_b), (param_value_b, param_name_b) = list(
            extreme_configurations.items()
        )[1]
        ...
        if values_0_a > values_0_b:
            modularity_config = {
                "values": (values_0_a, values_1_a),
                "param": (param_value_a, param_name_a),
            }
            enrichment_config = {
                "values": (values_0_b, values_1_b),
                "param": (param_value_b, param_name_b),
            }
        else:
            modularity_config = {
                "values": (values_0_b, values_1_b),
        ...
        for config_type, config_data in [
            ("max_modularity", modularity_config),
            ("max_enrichment", enrichment_config),
        ]:
            ...
            output_path: os.PathLike = os.path.join(
                args.results_folder,
                output_folder,
                f"{algorithm_name}_{param_value:.5f}.pdf",
            )
            ...
            legend = {
                "handles": [
                    Line2D(
                        ...
                        [0],
                        color="black",
                        label=f"Modularidad (Q): {values_0:.4f}",
                    ),
                    Line2D(
                        [0],
                        [0],
                        color="black",
                        label=f"P. Enriquecimiento Funcional (FES): {values_1:.4f}",
                    ),
            ...
            saved_clustering_paths[algorithm_name][param_value] = (
                output_path,
                param_name,
            )
    ...
    if VERBOSE:
        print(
            "Executing Fast-Greddy Algorithm (baseline). This may take a couple of minutes ..."
        )
        start = time.time()
    # Separate visualization for "Fast Greedy"
    fastgreedy_algorithm_name = "fastgreedy"
    results: List[List[int]] = Algorithms.fastgreedy_clustering(
    ...
    enrichment_score: float = Metrics.functional_enrichment_score(
        graph=graph, clusters=results, logger=logger
    )

    if VERBOSE:
        print(
            f"Fast-Greddy successfully computed and performance metrics calculated in {time.time() - start} seconds."
        )

    # Define the output path for "Fast Greedy"
    fastgreedy_output_path: os.PathLike = os.path.join(
        output_dir, f"{fastgreedy_algorithm_name}.pdf"
    )
    ...
    fastgreedy_legend = {
        "handles": [
            ...
            Line2D(
                ...
                [0],
                color="black",
                label=f"P. Enriquecimiento Funcional(FES): {enrichment_score:.4f}",
            ),
            Line2D([0], [0], color="black", label=f"Clusters: {num_clusters}"),
        ],
        "labels": [],  # Labels are embedded directly in the handles
    ...
    if VERBOSE:
        print(
            f"Final clustering results saved as a json file in {args.results_folder}/clustering_results.json."
        )

    save_clustering_results_as_json(
        graph=graph,
    ...
    for algorithm_name, param_data in saved_clustering_paths.items():
        ...
        plot_saved_clustering_results(
            saved_clustering_paths=single_algorithm_data, base_path_plots=PATH_PLOTS
        )

    if VERBOSE:
        print(
            f"Clustering analysis completed. All plots have been successfully generated."
        )
